...
void test7()
{
	...
	for (i = 0; i < num; i++)
	{
		bufMgr->allocPage(file2ptr, pid[i], page);
		sprintf((char*)tmpbuf, "test.7 Page %d %7.1f", pid[i], (float)pid[i]);
		rid[i] = page->insertRecord(tmpbuf);
		bufMgr->unPinPage(file2ptr, pid[i], true);
	}

	//flush file into disk
	bufMgr->flushFile(file1ptr);

	//read back pages
	for (i = 0; i < num; i++)
	{
		bufMgr->readPage(file2ptr, pid[i], page);
		sprintf((char*)&tmpbuf, "test.7 Page %d %7.1f", pid[i], (float)pid[i]);
		if(strncmp(page->getRecord(rid[i]).c_str(), tmpbuf, strlen(tmpbuf)) != 0)
		{
			PRINT_ERROR("ERROR :: CONTENTS DID NOT MATCH");
...
void test8()
{
	...
	//allocate pages more than available space
	for (i = 0; i < 2 * num; i++)
	{
		if (i < num)
		{
			bufMgr->allocPage(file1ptr, pid[i%num], page);
			sprintf((char*)tmpbuf, "test.8 Page %d %7.1f", pid[i%num], (float)pid[i%num]);
			rid[i%num] = page->insertRecord(tmpbuf);
			// bufMgr->unPinPage(file1ptr, pid[i%num], true);
		}
		else
		{
			PageId tmp;
			try 
			{
				bufMgr->allocPage(file1ptr, tmp, page);
			}
			catch (BufferExceededException e)
			{
	...
	for (i = 0; i < num; i++)
	{
		bufMgr->readPage(file1ptr, pid[i%num], page);
		sprintf((char*)&tmpbuf, "test.8 Page %d %7.1f", pid[i%num], (float)pid[i%num]);
		if(strncmp(page->getRecord(rid[i%num]).c_str(), tmpbuf, strlen(tmpbuf)) != 0)
		{
			PRINT_ERROR("ERROR :: CONTENTS DID NOT MATCH");
		}
		bufMgr->unPinPage(file1ptr, pid[i%num], false);
		bufMgr->unPinPage(file1ptr, pid[i%num], false);
	}
...
{ 
	...
	for (i = 0; i < num / 2; i++)
	{
		bufMgr->disposePage(file4ptr, pid[i]);
	}
...
void test10()
{
	...
	// allocate pages in another file
	for (i = num / 2; i < num; i++)
	{
		bufMgr->allocPage(file5ptr, pid[i], page);
		bufMgr->unPinPage(file5ptr, pid[i], true);
	}
...
void test11()
{
	...
	bufMgr->allocPage(file5ptr, pid[0], page);
	bufMgr->unPinPage(file5ptr, pid[0], false);
	try
	{
		bufMgr->unPinPage(file5ptr, pid[0], true);
	}
	catch (PageNotPinnedException e) 
	{
...
void test12()
{
	...
	bufMgr->allocPage(file1ptr, pid[1], page);

	// unpin with dirty bit set
	bufMgr->unPinPage(file1ptr, pid[0], true);
	// unpin without dirty bit set
...
void testextra()
{
	...
	for (i = 0; i < num/2; i++) {
		...
		sprintf((char*)tmpbuf, "test.5 Page %d %7.1f", pid[i], (float)pid[i]);
		rid[i] = page->insertRecord(tmpbuf);
		bufMgr->readPage(file5ptr, pid[i], page);
		bufMgr->readPage(file5ptr, pid[i], page);
		bufMgr->unPinPage(file5ptr, pid[i], true);
	...
	std::cout << "Passes extra test cases!\n";
}
